...
describe('timeLapse', () => {
  ...
  it('respects allowedUnits filter', () => {
    ...
    expect(timeLapse(target, ['DAY', 'HOUR'])).toEqual({ unit: 'DAY', value: 10 });
    expect(timeLapse(target, ['HOUR'])).toEqual({ unit: 'HOUR', value: 240 });
  });

  it('returns smallest allowed unit with value 0 if SECOND not allowed', () => {
    const target = new Date(Date.now() + 100);
    expect(timeLapse(target, ['MINUTE'])).toEqual({ unit: 'MINUTE', value: 0 });
  });

  it('works with ISO string input', () => {
    const target = new Date(Date.now() + 2 * 24 * 60 * 60 * 1000);
    expect(timeLapse(target.toISOString())).toEqual({ unit: 'DAY', value: 2 });
  });
});
